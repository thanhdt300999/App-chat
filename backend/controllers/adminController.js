...
const createProduct = asyncHandler(async (req, res) => {
    try {
        const { name, description, stock, image, author, type, price, salePercent, salePrice } = JSON.parse(req.body.data);
...
const getAllOrders = asyncHandler(async (req, res) => {
    try {
        ...
        const [orders, total] = await Promise.all([
            Order.find(query).skip(Number(skip)).limit(Number(pageSize)).populate({
        ...
        await Promise.all(
            order.products.map(async item => {
                ...
                try {
                    ...
                    console.log(`Đã cập nhật ${result.nModified} sản phẩm với id ${product}`);
                } catch (err) {
